...
func GenerateCNPs(snap *snapshot.Snapshot, rules []SimpleRule) ([]snapshot.Policy, []Manifest, error) {
	...
	order := []string{}
	draftFor := func(wl snapshot.Workload, dir string) (*cnpDraft, error) {
		key := wl.Namespace + "/" + wl.Name + "/" + dir
		if d, ok := drafts[key]; ok {
			return d, nil
		}
		sel, err := pickSelector(wl.Labels)
		if err != nil {
			return nil, fmt.Errorf("workload %s/%s %w", wl.Namespace, wl.Name, err)
		}
		d := &cnpDraft{
			ns:       wl.Namespace,
	...
	for i, r := range rules {
		...
		if (sides == "both" || sides == "egress") && srcEnt == "" {
			...
			if dstEnt != "" {
				...
				peer, err := peerSelector(dstWL)
				if err != nil {
					return nil, nil, fmt.Errorf("rule %d to: %w", i+1, err)
				}
				sec["toEndpoints"] = []any{peer}
			}
			if ports != nil {
				sec["toPorts"] = ports
			}
			d, err := draftFor(srcWL, "egress")
			if err != nil {
				return nil, nil, fmt.Errorf("rule %d from: %w", i+1, err)
			}
		...
		if (sides == "both" || sides == "ingress") && dstEnt == "" {
			...
			if srcEnt != "" {
				...
				peer, err := peerSelector(srcWL)
				if err != nil {
					return nil, nil, fmt.Errorf("rule %d from: %w", i+1, err)
				}
				sec["fromEndpoints"] = []any{peer}
			}
			if ports != nil {
				sec["toPorts"] = ports
			}
			d, err := draftFor(dstWL, "ingress")
			if err != nil {
				return nil, nil, fmt.Errorf("rule %d to: %w", i+1, err)
			}
		...
		// A rule that produced no section is a mistake the user should see, not
		// a silent no-op: an entity source with sides=egress (entities have no
		// egress policy) or an entity destination with sides=ingress.
		if !emitted {
			return nil, nil, fmt.Errorf("rule %d (%s → %s, sides=%s) generates no policy: "+
				"an entity has no policy on that side — put the workload there or use sides=both",
				i+1, r.From, r.To, sides)
		}
	}
...
// the policy beyond the drafted rule. A workload with no selectable labels
// at all is an error: matchLabels:{} selects every pod in the namespace, and
// the generated manifest is what the user copies to git.
func pickSelector(lbls map[string]string) (map[string]string, error) {
	...
	if len(out) == 0 {
		return nil, fmt.Errorf("has no stable labels to select on (a generated policy would select the whole namespace)")
	}
	return out, nil
}
...
func peerSelector(wl snapshot.Workload) (map[string]any, error) {
	sel, err := pickSelector(wl.Labels)
	if err != nil {
		return nil, fmt.Errorf("peer %s/%s %w", wl.Namespace, wl.Name, err)
	}
	ml := map[string]string{"k8s:io.kubernetes.pod.namespace": wl.Namespace}
	for k, v := range sel {
		ml["k8s:"+k] = v
	}
	return map[string]any{"matchLabels": ml}, nil
}
